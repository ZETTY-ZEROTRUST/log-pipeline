...
v3 보강 (2026-05-19, Track B — degenerate baseline 해소):
    ...
    - B2 ASN 다양성: 5개 KR 캐리어 ASN 분산 (asn-map.yaml 기준 전부 cgnat_kr) →
      asn_user_diversity baseline 입체화. ip_class 는 cgnat_kr 통일이 정답
...
# 로 통일이 정답이고, B2 가 흩는 것은 ip_asn 이다 (asn_user_diversity baseline 용).
# ────────────────────────────────────────────────────────────────────────────
KR_CARRIERS = [
